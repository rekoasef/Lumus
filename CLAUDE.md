...
- ❌ No sumar a mano lo gastado contra un presupuesto — sale de `fetchSpentByCategory` (`src/lib/finance/budget-spend-data.ts`), que pasa los gastos en dólares a pesos con el blue del día del gasto. Sumado crudo, US$ 120 contaban como $ 120 y el panel y la pantalla de presupuestos daban porcentajes distintos
...
- ❌ No apagar ni tocar `CHECKOUT_ENABLED` (`src/lib/billing/plan.ts`) sin que el dueño lo pida — está prendido desde el 2026-09-17. **El primer cobro real cae el día que termina la prueba del primer suscriptor**: antes de esa fecha tienen que estar el monotributo, Vercel Pro (Hobby prohíbe el uso comercial) y lo legal de `H6`. Si alguien se suscribe, avisarle al dueño la fecha. Ver `docs/LANZAMIENTO.md`
- ❌ No volver a mostrar las carteras con acciones y cripto sin que el dueño lo pida — están **escondidas a propósito** desde el 2026-09-16 (la UI no lo convenció), con `PORTFOLIO_WALLETS_ENABLED` en `src/lib/finance/feature-flags.ts`. El código está entero y la migración `00033` ya está en prod: prender el flag la devuelve. Y si se toca algo del tema, decidir con `isPortfolioWallet` / `effectiveInvestmentMode`, nunca comparando `investment_mode` a mano — la pantalla y la API tienen que decir lo mismo. Ver `E2` en `docs/BACKLOG.md`
...
- ❌ No cambiar un texto legal (`src/lib/legal/documents.ts`) sin pensar si hay que subir `TERMS_VERSION` (`src/lib/legal/owner.ts`): subirla hace que **todos** acepten de nuevo al entrar. Y no escribir en esos textos nada que no sea cierto hoy (el 2026-09-17 decían "inscripto en el monotributo" antes de estarlo)
- ❌ No pedir login en los botones de arrepentimiento y de baja (`/api/consumer-requests`): son las rutas públicas a propósito, junto con el webhook, el cron y la baja de avisos. La identidad se verifica con el link al mail, y la baja se ejecuta por POST, nunca al abrir el link
